...
/* ══════════════════════════════════════════════════════════════
   ...
   ARMAZENAMENTO:
     sessionStorage → sessão de login (apaga ao fechar a aba)
     Supabase DB    → perfil e galeria (persistem entre dispositivos)
     Supabase Storage → imagens (bucket "images")
══════════════════════════════════════════════════════════════ */

const SESSION_KEY = 'lhc_admin_s'

// Credenciais fixas — para alterar, mude os valores abaixo
const CREDS = { user: 'lidiani', pass: 'Santarita22@' }
...
export function AdminProvider({ children }) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (profileRes.data) setProfileSt(mapProfile(profileRes.data))
      if (galleryRes.data) setGallerySt(galleryRes.data.map(mapJob))
      setDbLoading(false)
    }
    fetchData()
  }, [])
